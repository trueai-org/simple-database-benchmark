...
public class FreeSqlBenchmarkService : IBenchmarkService
{
    ...
    public async Task<BenchmarkResult> GroupAggregationAsync()
    {
        ...
        public async Task<BenchmarkResult> CreateIndexesAsync()
        {
            var result = new BenchmarkResult
            {
                DatabaseName = DatabaseName,
                OperationType = "MillionData",
                OperationName = "CreateIndexes",
                TestTime = DateTime.Now
            };
            ...
            try
            {
                ...
                {
                    indexSql.AddRange(new[]
                    {
                        "CREATE INDEX IF NOT EXISTS idx_status ON test_entity(Status)",
                        "CREATE INDEX IF NOT EXISTS idx_category ON test_entity(Category)",
                        "CREATE INDEX IF NOT EXISTS idx_region_dept ON test_entity(Region, Department)",
                        "CREATE INDEX IF NOT EXISTS idx_status_cat_pri ON test_entity(Status, Category, Priority)",
                        "CREATE INDEX IF NOT EXISTS idx_score ON test_entity(Score)",
                        "CREATE INDEX IF NOT EXISTS idx_created_at ON test_entity(CreatedAt)",
                        "CREATE INDEX IF NOT EXISTS idx_salary ON test_entity(Salary)",
                        "CREATE INDEX IF NOT EXISTS idx_name ON test_entity(Name(50))",
                        "CREATE INDEX IF NOT EXISTS idx_priority ON test_entity(Priority)"
                    });
                }
                else if (DatabaseName == "SQLServer")
                {
                    indexSql.AddRange(new[]
                    {
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_status') CREATE INDEX idx_status ON test_entity(Status)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_category') CREATE INDEX idx_category ON test_entity(Category)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_region_dept') CREATE INDEX idx_region_dept ON test_entity(Region, Department)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_status_cat_pri') CREATE INDEX idx_status_cat_pri ON test_entity(Status, Category, Priority)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_score') CREATE INDEX idx_score ON test_entity(Score)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_created_at') CREATE INDEX idx_created_at ON test_entity(CreatedAt)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_salary') CREATE INDEX idx_salary ON test_entity(Salary)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_name') CREATE INDEX idx_name ON test_entity(Name)",
                        "IF NOT EXISTS (SELECT * FROM sys.indexes WHERE name='idx_priority') CREATE INDEX idx_priority ON test_entity(Priority)"
                    });
                }
                else if (DatabaseName == "PostgreSQL")
                {
                    indexSql.AddRange(new[]
                    {
                        ...
                        "CREATE INDEX IF NOT EXISTS idx_created_at ON test_entity(CreatedAt)",
                        "CREATE INDEX IF NOT EXISTS idx_salary ON test_entity(Salary)",
                        "CREATE INDEX IF NOT EXISTS idx_name ON test_entity(Name)",
                        "CREATE INDEX IF NOT EXISTS idx_priority ON test_entity(Priority)"
                    });
                }
                ...
                foreach (var sql in indexSql)
                {
                    try
                    {
                        await _freeSql.Ado.ExecuteNonQueryAsync(sql);
                    }
                    catch (Exception ex)
                    {
                ...
                result.RecordCount = indexSql.Count;
                result.ElapsedMilliseconds = monitor.ElapsedMilliseconds;
                result.OperationsPerSecond = monitor.CalculateOperationsPerSecond(indexSql.Count);
                result.CpuUsagePercent = monitor.CpuUsagePercent;
                result.MemoryUsedBytes = monitor.MemoryUsedBytes;
                result.MemoryUsedFormatted = monitor.MemoryUsedFormatted;
                result.IsSuccess = true;

                _logger.Information("[{Database}] 索引创建完成, 共 {Count} 个索引, 耗时: {Elapsed}ms", DatabaseName, indexSql.Count, result.ElapsedMilliseconds);
            }
            catch (Exception ex)
            {
                monitor.Stop();
                result.IsSuccess = false;
                result.ErrorMessage = ex.Message;
                _logger.Error(ex, "[{Database}] 索引创建失败", DatabaseName);
            }
