...
public class BenchmarkRunner
{
    ...
    public async Task InitializeServicesAsync()
    {
        ...
        /// 运行百万级数据索引测试
        /// </summary>
        public async Task RunMillionDataBenchmarksAsync()
        {
            var millionDataCount = _configuration.GetValue("BenchmarkSettings:MillionDataCount", 1000000);
            var millionBatchSize = _configuration.GetValue("BenchmarkSettings:MillionBatchSize", 10000);
            var indexQueryCount = _configuration.GetValue("BenchmarkSettings:IndexQueryCount", 100);
            var pageSize = _configuration.GetValue("BenchmarkSettings:PageSize", 100);
            var pageCount = _configuration.GetValue("BenchmarkSettings:PageCount", 100);

            _logger.Information("========== 开始百万级数据索引测试 ==========");
            _logger.Information("测试配置 - 数据量: {DataCount}, 批次大小: {BatchSize}, 查询次数: {QueryCount}, 分页大小: {PageSize}, 分页次数: {PageCount}",
                millionDataCount, millionBatchSize, indexQueryCount, pageSize, pageCount);

            foreach (var service in _services)
            {
                _logger.Information("");
                _logger.Information("========== 百万级测试数据库: {Database} ==========", service.DatabaseName);

                try
                {
                    // 1. 准备百万级测试数据
                    _logger.Information("[{Database}] ----- 准备百万级测试数据 -----", service.DatabaseName);
                    _allResults.Add(await service.PrepareMillionDataAsync(millionDataCount, millionBatchSize));

                    // 2. 创建索引
                    _logger.Information("[{Database}] ----- 创建索引 -----", service.DatabaseName);
                    _allResults.Add(await service.CreateIndexesAsync());

                    // 3. 索引查询测试
                    _logger.Information("[{Database}] ----- 索引查询测试 -----", service.DatabaseName);

                    // 主键查询
                    _allResults.Add(await service.IndexQueryByPrimaryKeyAsync(indexQueryCount));

                    // 单字段索引查询
                    _allResults.Add(await service.IndexQueryByStatusAsync(indexQueryCount));
                    _allResults.Add(await service.IndexQueryByCategoryAsync(indexQueryCount));

                    // 复合索引查询
                    _allResults.Add(await service.IndexQueryByRegionAndDepartmentAsync(indexQueryCount));
                    _allResults.Add(await service.IndexQueryByStatusCategoryPriorityAsync(indexQueryCount));

                    // 范围查询
                    _logger.Information("[{Database}] ----- 范围查询测试 -----", service.DatabaseName);
                    _allResults.Add(await service.IndexQueryByScoreRangeAsync(indexQueryCount));
                    _allResults.Add(await service.IndexQueryByDateRangeAsync(indexQueryCount));
                    _allResults.Add(await service.IndexQueryBySalaryRangeAsync(indexQueryCount));

                    // 前缀查询
                    _logger.Information("[{Database}] ----- 前缀查询测试 -----", service.DatabaseName);
                    _allResults.Add(await service.IndexQueryByNamePrefixAsync(indexQueryCount));

                    // 分页查询
                    _logger.Information("[{Database}] ----- 分页查询测试 -----", service.DatabaseName);
                    _allResults.Add(await service.IndexQueryWithPaginationAsync(pageSize, pageCount));

                    // 排序查询
                    _logger.Information("[{Database}] ----- 排序查询测试 -----", service.DatabaseName);
                    _allResults.Add(await service.IndexQueryWithOrderByAsync(indexQueryCount));

                    // 复杂条件查询
                    _logger.Information("[{Database}] ----- 复杂条件查询测试 -----", service.DatabaseName);
                    _allResults.Add(await service.IndexQueryComplexConditionAsync(indexQueryCount));

                    // 无索引查询对比
                    _logger.Information("[{Database}] ----- 无索引查询对比测试 -----", service.DatabaseName);
                    _allResults.Add(await service.NoIndexQueryAsync(Math.Min(indexQueryCount, 10))); // 全表扫描较慢，减少次数

                    // 4. 百万级数据聚合测试
                    _logger.Information("[{Database}] ----- 百万级数据聚合测试 -----", service.DatabaseName);
                    _allResults.Add(await service.MillionDataAggregationAsync());
                    _allResults.Add(await service.MillionDataGroupByAsync());
                ...
                {
                    _logger.Error(ex, "[{Database}] 百万级测试过程中发生错误", service.DatabaseName);
                }
            }

            _logger.Information("");
            _logger.Information("========== 百万级数据索引测试完成 ==========");
        }
